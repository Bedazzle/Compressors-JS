/*
 * lgk-js — LgK v1.1rs ZX Spectrum image compressor/decompressor in JavaScript
 * Original C implementation by Lethargeek
 * JavaScript port by Bedazzle, 2026 (compress + decompress only, no SFX generator).
...
function compressPicture(ctx) {
    ...
    if (ctx.cfg[CFRPR] === YES)
        if ((ctx.repeatRefs * ctx.cfg[CFREB]) < (ctx.modeTotals[XREQU] + ctx.modeTotals[XRINV])) {
            ctx.cfg[CFRPR] = NO;
            // Rebuild the model after disabling CFRPR so the emitted Huffman tables,
            // XOR buffer and header all agree (a bare minsize estimate adjustment
            // left an inconsistent bitstream that mis-depacked on some screens).
            buildModeCosts(ctx, 0);
